module tb_march_bist_top;
  ...
  march_bist_controller #(
      ...
      .memaddr_o(memaddr),
      .wdata_o  (wdata),
      .membm_o  (membm),   // <--- FIX 1: Connected missing port
      .memen_o  (memen),
      .memren_o (memren),
      .memwen_o (memwen)
  );
  ...
  // Pass Parameters at Elaboration Time (NOT using runtime force)
  // ---------------------------------------------------------------------------
  ...
  SRAM_1P_behavioral_bm_bist #(
    ...
    .FAULT_TARGET_ADDR (FAULT_ADDR),   // Target Address
    .EN_SAF            (1'b1),         // Enable Stuck-At Fault
    .SAF_VAL           (32'h0000_0000) // Forces bits at FAULT_ADDR to 0
  ) u_sram_mem (
      .A_ADDR    (memaddr),
      .A_DIN     (wdata),
      .A_BM      (membm_32b),
